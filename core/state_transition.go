...
import (
	...
	cmath "github.com/qydata/go-ctereum/common/math"
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.ChainConfig().IsImplAuth(st.evm.Context.BlockNumber) {
		//
		if msg.GasTipCap().Int64() > 0 {
			if !st.evm.ChainConfig().IsFixedGasPrice(msg.GasTipCap()) {
				log.Info("IsFixedGasPrice=======:", msg.GasTipCap())
				return nil, fmt.Errorf("%w: have %d, want %d", ErrFundsGasPriceMoreThan, msg.GasFeeCap(), st.evm.ChainConfig().FixedGasPrice())
			}
		}
